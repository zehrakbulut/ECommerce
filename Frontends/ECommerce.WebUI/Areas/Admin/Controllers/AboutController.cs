...
namespace ECommerce.WebUI.Areas.Admin.Controllers
{
    ...
    public class AboutController : Controller
    {
        ...
        public async Task<IActionResult> CreateAbout(CreateAboutDto createAboutDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7090/api/Abouts", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateAbout(string id)
        {
            ...
            ViewBag.v0 = "Hakkımda İşlemleri";
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7090/api/Abouts/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateAbout(UpdateAboutDto updateAboutDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7090/api/Abouts/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "About", new { area = "Admin" });
            }
